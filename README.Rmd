...
-   Imputing and formatting full and partial dates: see [`vignette("Dates", "sdtmval")`](https://skgithub14.github.io/sdtmval/articles/dates.html)
...
In this example work flow, we will load in a raw EDC table and transform it into a SDTM domain table. We will use the made-up domain 'XX' and some example data included with {sdtmval}.
...
Next, using codelist we retrieved from the spec earlier, we can create the `XXTEST` variable.
...
Next, we will assign the sequence number using `assign_SEQ()`. This function also sorts the data set by whatever you make the `key_vars` argument.
